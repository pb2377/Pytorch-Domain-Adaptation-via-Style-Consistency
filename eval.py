...
def test_net(output_dir, net, dataset, set_type, use_07=True, conf_thresh=0.01, overlap_thresh=0.45):
    ...
    for i in range(num_images):
        im, im_viz, gt, h, w = dataset.pull_item(i)
...
def evaluate_detections(box_list, output_dir, dataset, set_type, use_07=True, overlap_thresh=0.45):
    ...
    accuracy_dict = do_python_eval(output_dir, annopath, imgsetpath, dataset.target_domain, use_07=use_07,
                                   set_type=set_type, overlap_threshold=overlap_thresh)
    return accuracy_dict


def evaluate(model, dataset, save_folder, overlap_thresh=0.45):
