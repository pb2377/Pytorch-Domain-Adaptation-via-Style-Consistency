...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes, cfg):
        ...
        self.instn_indices = [7, 14, 21, 28, 33]
    ...
    def forward(self, x, encode_only=False):
        ...
        # apply vgg up to fc7 + conv6
        for k in range(23, len(self.vgg)):
            x = self.vgg[k](x)
            # # store output of Conv6 SSD extra
            if k == (len(self.vgg) - 3):
        ...
        aux_outputs.append(x)  # Conv7 SSD extra
...
def build_ssd(cfg, phase, size=300, num_classes=21, instn=False):
